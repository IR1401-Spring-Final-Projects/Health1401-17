...
class TfIdfRecommender():
  ...
  def recommend(self, query, k=10):
    ...
    better_rate = np.array([self.get_sim(modified_query, document_vectors[i]) for i in range(document_vectors.shape[0])], dtype=float) # calculating cosine similarity and rating
...
class TransformerRecommender():
  ...
  def recommend(self, query, k=10):
    ...
    irrelevant_docs = np.array([self.document_vectors[x] for x in irrelevants])
    modified_query = improve_query(relevant_docs, irrelevant_docs, query_vector)
    better_rate = list([util.cos_sim(modified_query, vector) for vector in self.document_vectors]) # calculating cosine similarity and getting the k highest scores
    better_similars = np.argpartition(better_rate, -k)[-k:]
    matched_documents = self.document_list.loc[better_similars, :]
    return matched_documents
...
class EmbedRecommender():
  ...
  def recommend(self, query, k=10):
    ...
    irrelevant_docs = np.array([self.document_vectors[x] for x in irrelevants])
    modified_query = improve_query(relevant_docs, irrelevant_docs, query_vector)
    better_rate = np.array([self.get_sim(modified_query, vector) for vector in self.document_vectors])
    better_similars = np.argpartition(better_rate, -k)[-k:]
    matched_documents = self.document_list.loc[better_similars, :]
...
class Initial:
  def __init__(self):
      ...
      # print(self.initial_elastic.search('کبد')['hits']['hits'][1]['_source'])
